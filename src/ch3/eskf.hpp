...
bool ESKF<S>::Predict(const IMU& imu) {
    ...
    // LOG(INFO) << "dx_original = "<<dx_.transpose();
    starttime = system_clock::now();
    // LOG(INFO) << "dx_ = "<<dx_.transpose();

    // 均值和协方差矩阵预测 mean and cov prediction
    dx_ = F * dx_;  // 这行其实没必要算，dx_在重置之后应该为零，因此这步可以跳过，但F需要参与Cov部分计算，所以保留

    diff = system_clock::now()- starttime;    
...
bool ESKF<S>::ObserveSE3(const SE3& pose, double trans_noise, double ang_noise) {
    ...
    Mat6d V = noise_vec.asDiagonal(); // 6维噪声向量转换为6x6噪声对角矩阵
    Eigen::Matrix<S, 18, 6> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + V).inverse();

    // 更新x和cov
    Vec6d innov = Vec6d::Zero();
    innov.template head<3>() = (pose.translation() - p_);          // 平移部分：p_gnss - p
    // innov.template tail<3>() = (R_.inverse() * pose.so3()).log();  // 旋转部分：Log(R^T * R_gnss)，见公式(3.67) 
    // 我的左乘模型修改 2
    innov.template tail<3>() = (pose.so3() * R_.inverse()).log();  // 旋转部分，见公式(3.67)

    LOG(INFO) << "SO3: " << R_.matrix();
    LOG(INFO) << "dx_ = "<<dx_.transpose();
