...
void MRLikelihoodField::BuildModel() {
    ...
    /// 生成模板金字塔图像
    field_ = {
        // 增加不同分辨率的似然场图像，如果初始位姿估计较差，那么激光投出来的点可能产生较大的误差，与动态物体产生的误差难以区分，容易被当成异常值剔除，使用小分辨率的似然场图像，那么每个点对应的误差也会变小，可以先在小分辨率的似然场中进行一次配准，然后把粗配准的结果投影到高分辨率的似然场中，形成由粗到精的匹配过程 coarse-to-fine
    ...
    for (int x = -range; x <= range; ++x) {
        for (int y = -range; y <= range; ++y) 
            // emplace_back()函数接受构造ModelPoint对象所需的参数，
            // 并在向量的末尾直接构造该对象，而无需先创建一个临时对象再将其复制或移动到向量中。
            // push_back()函数则必须创造一个临时对象，然后将其复制或移动到向量中。        
