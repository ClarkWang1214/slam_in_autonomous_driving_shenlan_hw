...
int main(int argc, char** argv) {
    ...
    rosbag_io
        .AddScan2DHandle(
            "/pavo_scan_bottom",
            [&](Scan2d::Ptr scan) {
                sad::LikelihoodField lf; // 构建似然场对象，调用默认构造函数，生成似然场模板
                current_scan = scan;     // 当前scan
                SE2 pose;   // SE2位姿
                ...
                lf.SetTargetScan(last_scan);    // 上一个scan作为目标
                lf.SetSourceScan(current_scan); // 当前scan作为源
                ...
                sad::Visualize2DScan(last_scan, SE2(), field_image, Vec3b(255, 0, 0), 1000, 20.0);  // target是蓝的
