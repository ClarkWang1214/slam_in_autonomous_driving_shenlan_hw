...
/**
 * @description: 添加新的子地图
 * @param {shared_ptr<Submap>} submap
 * @return {*}
 */
...
void LoopClosing::MatchInHistorySubmaps() {
    ...
    for (const size_t& can : current_candidates_) {
        auto submap = submaps_[can];                    // 获取当前子地图

        auto mr = submap_to_field_.at(submap);   // 获取当前子地图的似然场
        mr->SetSourceScan(current_frame_->scan_);       // 设置当前帧的scan

        SE2 pose_in_target_submap = submap->GetPose().inverse() * current_frame_->pose_;  // T_S1_C = T_S1_W * T_W_C = (T_W_S1)^-1 * T_W_C
        ...
        if (mr->AlignG2O(pose_in_target_submap)) {
            ...
            // 对应公式（6.24） T_S1_S2 = T_S1_C * T_C_W * T_W_S2 
            SE2 T_this_cur = pose_in_target_submap *                // 当前帧在子地图坐标系下的SE2位姿：T_S1_C
                             current_frame_->pose_.inverse() *      // 当前帧在世界坐标系下的SE2位姿T_W_C的逆：T_C_W
                             submaps_[last_submap_id_]->GetPose();  // 子地图在世界坐标系下的SE2位姿：T_W_S2
            ...
            cv::putText(occu_image, "loop submap " + std::to_string(submap->GetId()), cv::Point2f(20, 20), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 0));
            cv::putText(occu_image, "keyframes " + std::to_string(submap->NumFrames()), cv::Point2f(20, 50), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(0, 255, 0));
