 ...
 */
void PlaneFittingTest() {
    // 真实的平面系数，n \in R^3 为三维平面法向量，d \in R 为平面的截距 
    ...
    for (int i = 0; i < FLAGS_num_tested_points_plane; ++i) {
        ...
        Vec3d p(rng.uniform(0.0, 1.0), rng.uniform(0.0, 1.0), rng.uniform(0.0, 1.0));   // 生成三维随机点
        // 点p与真实平面系数（n,d）点积，n为平面法向量，d为平面的截距
        double n4 = -p.dot(true_plane_coeffs.head<3>()) / true_plane_coeffs[3];
        p = p / (n4 + std::numeric_limits<double>::min()); // 归一化为齐次坐标，最后一维为1，防止除零
        p += Vec3d(rng.gaussian(FLAGS_noise_sigma), rng.gaussian(FLAGS_noise_sigma), rng.gaussian(FLAGS_noise_sigma));  // 添加随机噪声
    ...
    if (sad::math::FitPlane(points, estimated_plane_coeffs)) 
        LOG(INFO) << "estimated coeffs: " << estimated_plane_coeffs.transpose() << ", true: " << true_plane_coeffs.transpose();
    else 
